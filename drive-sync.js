/* drive-sync.js — Bribox Kanpus (fixed) */
;(() => {
  ...
  function ensureScriptsReady(){
    return new Promise((resolve, reject)=>{
      let okGapi = !!(window.gapi && gapi.load);
      let okGIS  = !!(window.google?.accounts?.oauth2);
      const t = setInterval(()=>{
        okGapi ||= !!(window.gapi && gapi.load);
        okGIS  ||= !!(window.google?.accounts?.oauth2);
        if (okGapi && okGIS){ clearInterval(t); resolve(); }
      }, 30);
      setTimeout(()=>{ clearInterval(t); if(!okGapi) reject(new Error('gapi tidak siap')); else if(!okGIS) reject(new Error('GIS tidak siap')); }, 8000);
    });
  }
  ...
  async function afterToken(resp){
    if (!resp || resp.error || !resp.access_token){
      throw new Error(resp?.error_description || resp?.error || 'Gagal mendapatkan access_token');
    }
    accessToken = resp.access_token;
    gapi.client.setToken({ access_token: accessToken }); // 🔑 wajib
    const prof = await getProfile().catch(()=>null);
    cachedProfile = prof || cachedProfile;
  ...
  async function signIn(){
    ...
    return new Promise((resolve, reject)=>{
      try{
        tokenClient.callback = (resp)=>{ afterToken(resp).then(()=>resolve(resp)).catch(reject); };
  ...
  function signOut(){
    ...
    accessToken = null; cachedProfile = null;
    gapi.client.setToken(null);
    rootFolderId = null; subfolders.clear();
    setAuthUI(false);
  }
  function assertLogged(){ if (!accessToken) throw new Error('Belum login Google Drive.'); }

  // gunakan OIDC userinfo (sesuai scope openid/email/profile)
  ...
  async function queryDrive(q, fields){
    ...
    const res = await gapi.client.drive.files.list({ q, spaces:'drive', pageSize:1000, fields:'files('+f+')' });
  ...
  async function ensureRootFolder(){
    ...
    rootFolderId = res.result.id; return rootFolderId;
  }
  async function ensureSubfolder(mod){
    ...
    subfolders.set(mod, res.result.id); return res.result.id;
  }
  ...
  // Auto-wire tombol (tanpa silent login agar tidak "kedip")
