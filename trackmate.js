...
document.addEventListener('DOMContentLoaded', async () => {
  // Auto-pull cloud data when page becomes visible and Drive is connected
  const autoSync = async () => {
    try {
      ...
      if (ok || window.DriveSync?.isLogged?.()) {
        const cloud = await driveLoadHistoriManifest();
        if (Array.isArray(cloud)) {
          const local = readHistoriSafe();
          // Simple merge - prefer cloud for conflicts
          const mergedMap = new Map();
          
          local.forEach(item => {
            if (item.contentHash) mergedMap.set(item.contentHash, item);
          });
          
          cloud.forEach(item => {
            if (item.contentHash) {
              const existing = mergedMap.get(item.contentHash);
              if (!existing || (item.uploadedAt > existing.uploadedAt)) {
                mergedMap.set(item.contentHash, item);
              }
            }
          });
          ...
          if (merged.length !== local.length) {
            writeHistori(merged);
            console.log('[Trackmate] Synced from cloud:', merged.length, 'items');
          }
        }
      }
    } catch (e) {
      console.warn('[Trackmate] Auto-sync failed:', e);
    }
  };
